...
// Get all the notes for a single user TR
  app.get("/getAllNotes", express.json(), async (req, res) => {
    try {
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
        if (err) {
          return res.status(401).send("Unauthorized. You do not have access to notes related to this ID.");
        }
  
        // Look through the db for all the user's notes
        const collection = db.collection(COLLECTIONS.notes);
        const data = await collection.find({ username: decoded.username });
        
        // array to store all the user's notes
        let usersNotes = [];
        for await (const note of data) {
            usersNotes.push(note)
        }

        // return the status 200 along with notes to show the request was completed properly
        // an empty array can be returned if the user has no notes
        return res.status(200).json({ response: usersNotes });
      });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
...
// Delete a note for a single user AH
app.delete("/deleteNote/:noteId", express.json(), async (req, res) => {
    try {
      const noteId = req.params.noteId;
      if (!ObjectId.isValid(noteId)) {
        return res.status(400).json({ error: "This note ID is invalid." });
      }
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
        if (err) {
          return res.status(401).send("Unauthorized. You do not have access");
        }
        ...
        if (data.deletedCount === 0 || !data) {
            return res.status(404).json({ error: "Unable to find note with this ID belonging to this user" });
        }

        return res.status(200).json({ response: `Note with ID ${noteId} has been deleted` });
      });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
// Editing a note with a specific ID AH
app.patch("/editNote/:noteId", async (req, res) => {
    try {
        ...
        if (title === null || content === null || title === undefined || content === undefined) {
        ...
        jwt.verify(token, "secret-key", async (err, decoded) => {
            ...
            const collection = db.collection(COLLECTIONS.notes);
            const noteData = await collection.updateOne(
            {
                username: decoded.username,
                _id: new ObjectId(noteId),
            },
            ...
            if (noteData.modifiedCount === 0 || !noteData) {
                return res.status(404).json({ error: "Unable to find note with this ID belonging to this user" });
              }
            return res.status(200).json({ response: `Note with ID ${noteId} has been updated.`});
          });
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
